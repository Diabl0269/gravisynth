...
public:
  LFOModule() : ModuleBase("LFO", 0, 2) { // 0 Audio Inputs, 2 Control Outputs
    ...
    addParameter(modeParam =
                     new juce::AudioParameterBool("mode", "Sync", true));

    // Bipolar: Unipolar (false) / Bipolar (true)
    addParameter(bipolarParam =
                     new juce::AudioParameterBool("bipolar", "Bipolar", true));
    ...
    addParameter(rateSyncParam = new juce::AudioParameterChoice(
                     "rateSync", "Sync Rate", syncs, 2)); // Default 1/4

    // Retrig
    addParameter(retrigParam =
                     new juce::AudioParameterBool("retrig", "Retrig", false));
  ...
  void processBlock(juce::AudioBuffer<float> &buffer,
    ...
    for (int sample = 0; sample < buffer.getNumSamples(); ++sample) {
      ...
      case 1: // Triangle
        currentSample =
            2.0f * std::abs(2.0f * (phase - std::floor(phase + 0.5f))) - 1.0f;
        break;
      case 2: // Sawtooth
