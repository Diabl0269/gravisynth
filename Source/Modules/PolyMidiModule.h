...
public:
  PolyMidiModule() : ModuleBase("Poly MIDI", 0, 2) {
...
private:
  ...
  void renderChunk(juce::AudioBuffer<float> &buffer, int startSample,
                   int endSample) {
    ...
    for (int i = 0; i < MAX_VOICES; ++i) {
      ...
      float freq =
          voices[i].currentFreq; // Use stored freq (holds during release)
  ...
  void handleNoteOn(int note, float velocity) {
    ...
    for (int i = 0; i < MAX_VOICES; ++i) {
      if (voices[i].active && voices[i].note == note) {
        ...
        voices[i].currentFreq = juce::MidiMessage::getMidiNoteInHertz(
            note); // Update freq in case of pitch bend
